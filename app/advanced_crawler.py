...
from utils.text_processor import clean_html_text, simple_tokenize
...
async def upsert_page(session: AsyncSession, site_id: int, url: str, metadata: Dict) -> int:
    ...
    result = await session.execute(
        text("""
            INSERT INTO pages (
                url, site_id, title, content,
                og_title, og_description, og_image_url, jsonld,
                last_crawled_at
            )
            VALUES (:url, :site_id, :title, :content, :og_title, :og_desc, :og_img, :jsonld, NOW())
...
async def crawl_recursive(
    ...
    """Advanced recursive crawler with:
    - Async concurrency
    - URL normalization
    - Robots.txt respect
    - Rate limiting
