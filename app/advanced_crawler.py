...
from utils.text_processor import clean_html_text
...
def extract_metadata(soup: BeautifulSoup, url: str) -> Dict:
    ...
    for img in soup.find_all("img"):
        src = img.get("src")
        if src:
            images.append({"url": urljoin(url, src), "alt": img.get("alt", "")[:255]})
...
async def register_site(session: AsyncSession, domain: str, site_base: str) -> int:
    result = await session.execute(text("SELECT id FROM sites WHERE domain = :d"), {"d": domain})
    row = result.fetchone()
    if row:
        return row.id
    ...
    async with httpx.AsyncClient(headers=headers, follow_redirects=True) as client:
        ...
        while queue and stats.total_success < max_pages:
            ...
            while (
                ...
                and stats.total_success + len(batch) < max_pages
            ):
...
if __name__ == "__main__":
    ...
    asyncio.run(
        crawl_recursive(start, max_pages=max_pages, max_depth=max_depth, concurrency=concurrency)
    )
